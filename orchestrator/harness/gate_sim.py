...
def udp_shim_source(prefixes: tuple = ("sky130_fd_sc_hd",)) -> str:
    """Verilator-compatible behavioural replacements for the standard-cell UDPs.

    The SHIM TEXT is now owned by the active deployment (PR6, byo-pdk): the
    sky130 deployment transcribes each ``primitive`` truth table from the PDK's
    ``primitives.v`` (port order matches exactly; sequential cells use
    non-blocking assignment so zero-delay gate sim is race-free). A BYO
    deployment supplies its own (or none). This stays the stable gate-sim entry
    point + ``__all__`` export; ``_SHIM_UDPS`` above documents the covered set.

    We ship replacements rather than compiling ``primitives.v`` because
    Verilator cannot compile Verilog-1995 UDP ``table`` bodies, and its
    ``--bbox-unsup`` escape hatch would black-box every flop/latch -- a
    guaranteed false PASS.
    """
    from orchestrator.pdk.registry import get_deployment
    return get_deployment().udp_shim_source(prefixes)
...
def cell_model_files(
    ...
    Delegates to the active deployment (PR6), which owns the cell-library list
    and discovery. Returns ``(files, prefixes)`` -- only the libraries the
    netlist references when ``netlist_text`` is given -- or ``([], ())`` when
    the PDK is absent; the caller degrades to ``not_run``, never to a pass.
    """
    from orchestrator.pdk.registry import get_deployment
    root = str(pdk_root) if pdk_root is not None else None
    return get_deployment().cell_model_files(netlist_text=netlist_text,
                                             pdk_root=root)
...
# The Verilator defines are owned by the deployment (PR6); this stays as the
# sky130 default + a compat constant. The build path reads sim_defines() below.
VERILATOR_DEFINES = ["FUNCTIONAL", "UNIT_DELAY="]


def _sim_defines() -> list[str]:
    """Verilator ``-D`` defines from the active deployment (falls back to the
    sky130 default constant if resolution fails, so a build never breaks)."""
    try:
        from orchestrator.pdk.registry import get_deployment
        d = get_deployment().sim_defines()
        return list(d) if d else list(VERILATOR_DEFINES)
    except Exception:  # noqa: BLE001
        return list(VERILATOR_DEFINES)
