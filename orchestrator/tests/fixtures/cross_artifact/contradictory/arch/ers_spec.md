...
<!--
Fixture excerpt. Derived from a real architecture-phase run's
arch/ers_spec.md.bak-chiplead (the pre-correction copy). Two defects are
preserved verbatim in shape:
  * the host operating point (6.25 MHz / 25 Mbit/s / 80 ns) DISAGREES with the
    FRD fixture next to it, which still says 12.5 MHz / 50 Mbit/s / 40 ns;
  * the READ-serializer requirement below mandates RISE-scheduled nibble
    presentation, while the per-block qspi_slave_engine requirement further
    down says the serializer advances on sck_fall.
-->
...
- For READ, issue the first aperture request early enough to capture its one-cycle response during dummy, then maintain current/next byte registers and valid bits so every sck_fall in data phase updates one nibble without a bubble.
