...
# ---------------------------------------------------------------------------
# Gate-sim data (PR6): the sky130 deployment OWNS its standard-cell simulation
# models, the Verilator-compatible UDP shim text, and the Verilator defines.
# Moved verbatim from harness/gate_sim.py (which now delegates here).
# ---------------------------------------------------------------------------
# Standard-cell families we know how to shim, in preference order.
_CELL_LIBRARIES = ("sky130_fd_sc_hd", "sky130_fd_sc_hvl", "sky130_fd_sc_ms",
                   "sky130_fd_sc_ls", "sky130_fd_sc_lp", "sky130_fd_sc_hs")

# Verilator defines: FUNCTIONAL selects the behavioural cell models;
# UNIT_DELAY= (empty) strips the PDK's #UNIT_DELAY annotations for zero-delay,
# race-free replay.
_VERILATOR_DEFINES = ["FUNCTIONAL", "UNIT_DELAY="]

# The behavioural stand-ins for the ten sky130 UDP primitives referenced by the
# cell models under -DFUNCTIONAL with USE_POWER_PINS undefined. Each body is a
# direct transcription of the primitive's truth table; port ORDER matches
# primitives.v exactly (the cells bind positionally). Sequential cells use
# non-blocking assignment -> zero-delay gate sim is race-free.
_SHIM_TEMPLATE = """
...
def _udp_shim_source(prefixes: tuple) -> str:
    parts = [
...
# Macro discovery defaults (PR6): sky130 SRAM/ROM macros live under this leaf
# dir inside a PDK variant. efabless ships them in sky130B; some installs mirror
# them into sky130A. vccd1/vssd1 are the power/ground pin fallbacks.
_SRAM_DIR = "sky130_sram_macros"
_MACRO_VARIANTS = ("sky130A", "sky130B")
_MACRO_LIB_CORNER = "TT_1p8V_25C"
_MACRO_POWER_PIN = "vccd1"
_MACRO_GROUND_PIN = "vssd1"
...
# ---------------------------------------------------------------------------
# The deployment
class GenMacroOpenram(EdaTool):
    """``gen_macro`` -> OpenRAM memory-macro generation (PR6).

    Present in the sky130 deployment's capabilities ONLY when OpenRAM can
    actually be invoked (``openram_gen.openram_available()``); otherwise the CLI
    reports an honest capability skip (exit 4) -- never a false green. Wraps
    ``openram_gen.ensure_macro`` (which resolves an exact pre-built part or
    generates one), so ``bin/gen_ram`` and this verb share one code path.
    """

    verb: ClassVar[str] = "gen_macro"
    ...
    def run(self, req: ToolRequest) -> ToolResult:
        ...
        try:
            ...
            return ToolResult.from_checks(
                tool_ok=False,
                checks=[CheckResult("inputs", "fail",
                                    details="gen_macro needs --width and --depth")],
                verb=self.verb, design=req.design)
        try:
            ...
            return ToolResult.from_checks(
                tool_ok=False,
                checks=[CheckResult("gen_macro", "fail",
                                    details=f"ensure_macro raised: {exc}")],
                verb=self.verb, design=req.design)
        if isinstance(res, MacroInfo):
            artifacts = {k: Path(getattr(res, k)) for k in
                         ("lef", "gds", "lib", "spice", "verilog")
                         if getattr(res, k, "")}
            metrics = {"name": res.name, "bits": res.bits,
                       "data_bits": res.data_bits, "words": res.words,
                       "ports": res.ports}
            return ToolResult.from_checks(
                tool_ok=True,
                checks=[CheckResult("gen_macro", "pass",
                                    details=f"resolved macro {res.name}")],
                artifacts=artifacts, verb=self.verb, design=req.design,
                extra_metrics=metrics)
        # A CompositionPlan (tile pre-built macros) or None: no single macro was
        # generated. tool ran, but the verb did not produce a macro -> fail
        # (never a false pass; the caller decides whether to tile in RTL).
        detail = ("no single macro for the geometry -- OpenRAM did not produce "
                  "one" if res is None else f"composition plan required: {res}")
        return ToolResult.from_checks(
            tool_ok=True,
            checks=[CheckResult("gen_macro", "fail", details=detail)],
            verb=self.verb, design=req.design)

    def prompt_notes(self) -> str:
        return ("OpenRAM SRAM/ROM macro generation on sky130 (the repaired "
                "wheel). Give --width/--depth/--ports; the deployment resolves "
                "an exact pre-built part or generates one.")
...
class Sky130Deployment(Deployment):
    ...
    def __init__(self) -> None:
        ...
        self._gen_macro: GenMacroOpenram | None = None
        self._pdk: PDKConfig | None = None

    # -- gen_macro capability (PR6): lazily gated on OpenRAM availability so the
    # hot paths (tools()["run_pnr"] etc.) never trigger the OpenRAM probe/patch.
    # gen_macro is exposed via supports()/capabilities()/tool(), NOT the base
    # _tools map, so a plain tools() lookup stays a cheap dict read.
    @staticmethod
    def _openram_available() -> bool:
        try:
            from orchestrator.langgraph.openram_gen import openram_available
            return openram_available()
        except Exception:  # noqa: BLE001
            return False

    def _gen_macro_tool(self) -> GenMacroOpenram:
        if self._gen_macro is None:
            self._gen_macro = GenMacroOpenram(self)
        return self._gen_macro

    def supports(self, verb: str) -> bool:
        if verb in self._tools:
            return True
        if verb == "gen_macro":
            return self._openram_available()
        return False

    def capabilities(self) -> set[str]:
        caps = set(self._tools)
        if self._openram_available():
            caps.add("gen_macro")
        return caps

    def tool(self, verb: str):
        if verb in self._tools:
            return self._tools[verb]
        if verb == "gen_macro" and self._openram_available():
            return self._gen_macro_tool()
        return None
    ...
    def cell_model_files(self, netlist_text: str = "",
        ...
        """Locate the PDK standard-cell SIMULATION models (not blackbox views).

        Returns ``(files, prefixes)``; only the libraries the netlist actually
        references (when ``netlist_text`` is given). ``([], ())`` when the PDK
        is absent -> caller degrades to ``not_run``, never a pass."""
        if pdk_root is None:
            pdk_root = str(self._pdk_root)
        root = Path(pdk_root)
        wanted = [
            lib for lib in _CELL_LIBRARIES
            if (not netlist_text) or re.search(rf"\b{re.escape(lib)}__", netlist_text)
        ]
        if not wanted:
            wanted = [_CELL_LIBRARIES[0]]
    ...
    def udp_shim_source(self, prefixes: tuple = ("sky130_fd_sc_hd",)) -> str:
        return _udp_shim_source(prefixes)

    def sim_defines(self) -> list[str]:
        return list(_VERILATOR_DEFINES)

    # -- macro accessors (PR6) ------------------------------------------
    def macro_search_paths(self, pdk_root: str) -> list[Path]:
        root = Path(pdk_root)
        return [root / v / "libs.ref" / _SRAM_DIR for v in _MACRO_VARIANTS]

    def macro_defaults(self) -> dict[str, str]:
        return {"power_pin": _MACRO_POWER_PIN, "ground_pin": _MACRO_GROUND_PIN,
                "lib_corner": _MACRO_LIB_CORNER}

    def gds_layer_map(self):
        # The sky130 GDS layer map lives in the architecture viewer as the
        # engine default; returning None lets the viewer use it unchanged.
        return None
