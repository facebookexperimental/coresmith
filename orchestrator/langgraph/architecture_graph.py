...
Document hierarchy:
    ...
    ERS  -- "What is needed to enable it?"         (Engineering Requirements)
...
Flow:
    START -> Gather Requirements (LLM) -> route_after_prd
          ...
          -> Engineering Requirements (ERS) -> Constraint Check -> route_after_constraints
...
async def engineering_requirements_node(state: ArchGraphState) -> dict:
    """Synthesize + persist the ERS so Constraint Check can gate on it.

    Writes ``.coresmith/ers_spec.json`` and ``arch/ers_spec.md`` (via
    ``_persist_ers``) and returns the result in ``ers_spec`` so
    ``constraint_check_node`` -- which already threads ``state["ers_spec"]``
    into ``check_constraints`` -- has something to thread.

    Generation failure is non-fatal (same as the pre-move behavior): the run
    continues without an ERS, the cross-artifact gate simply has one fewer
    artifact, and ``create_documentation_node`` still has its fallback path.
    """
    _event(state, "Engineering Requirements", "graph_node_enter", {
        "round": state["round"],
    })

    with _tracer.start_as_current_span("Engineering Requirements") as span:
        span.set_attribute("round", state["round"])

        if not _ers_before_constraints_enabled():
            span.set_attribute("skipped", True)
            _event(state, "Engineering Requirements", "graph_node_exit", {
                "round": state["round"],
                "skipped": "CORESMITH_ERS_BEFORE_CONSTRAINTS=0",
            })
            return {"phase": "ers"}

        project_root = state["project_root"]
        block_diagram = state.get("block_diagram") or {}
        if not block_diagram.get("blocks"):
            span.set_attribute("skipped", True)
            _event(state, "Engineering Requirements", "graph_node_exit", {
                "round": state["round"], "skipped": "no_block_diagram",
            })
            return {"phase": "ers"}
        ...
        _event(state, "Engineering Requirements", "graph_node_exit", {
            "round": state["round"], "ers_generated": True,
        })
        update = {"ers_spec": ers_result, "phase": "ers"}
        _persist_intermediate_state(state, update)
        return update
...
# interrupt-driven (gather_prd asks the human), and the ERS is *derived* -- it
# is now emitted before the gate (Engineering Requirements), but re-issuing it
# alone would just re-synthesize from the same PRD/SAD/FRD, so SAD and FRD stay
# the in-scope regenerable prose docs. Doc Fix now routes through Engineering
# Requirements, so a repaired FRD does refresh the ERS. Any other source_doc
# routes to the existing Block Diagram path unchanged.
...
async def create_documentation_node(state: ArchGraphState) -> dict:
    ...
    with _tracer.start_as_current_span("Create Documentation") as span:
        ...
        # The ERS is emitted by the Engineering Requirements node, BEFORE the
        # constraint gate, so the gate can actually read it. Re-generating it
        # here would (a) double-write the file and (b) silently ship an ERS
        # that differs from the one the gate approved -- which would make the
        # gate a lie. Reuse it; only generate as a fallback (gate disabled, or
        # the earlier attempt failed).
        ers_result = state.get("ers_spec") or None
        if ers_result:
            span.set_attribute("ers_generated", False)
            span.set_attribute("ers_reused_from_gate", True)
        else:
            try:
                from orchestrator.architecture.specialists.ers_doc import (
                    generate_ers_doc,
...
def build_architecture_graph(checkpointer=None):
    ...
    # Doc Fix regenerates the offending SAD/FRD, then re-synthesizes the ERS
    # from it before re-checking constraints. Routing straight back to
    # Constraint Check would leave the ERS derived from the OLD FRD, so an
    # FRD/ERS disagreement would be re-reported every round forever.
    graph.add_edge("Doc Fix", "Engineering Requirements")
