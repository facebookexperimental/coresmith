...
def _phase_evidence_section(block_name: str, phase: str) -> tuple[str, str]:
    ...
    files = (
        ...
        f"- uArch spec: arch/uarch_specs/{block_name}.md\n"
        f"- Frozen interface contract: .coresmith/interface_contracts.json\n"
        f"- Constraints: .coresmith/blocks/{block_name}/constraints.json\n"
        f"- Attempt history: .coresmith/blocks/{block_name}/attempt_history.json\n"
        f"- Block diagram: .coresmith/block_diagram.json (for interface context)\n"
        f"- ERS: arch/ers_spec.md\n\n"
    ...
    instructions = (
        f"## Instructions\n"
        f"1. Read the first failure in the error/step log, then the relevant source context.\n"
        f"2. Compare the failing behavior with the relevant uArch/ERS/interface contract.\n"
        f"3. Expand source reads or inspect a focused VCD interval only to resolve an open hypothesis.\n"
        f"4. If a reproduction is needed, use a fresh subdirectory under "
        f".coresmith/debug_scratch/{block_name}/ with copied inputs and explicit "
        f"build/log destinations. Preserve canonical sources and original failure artifacts. "
        f"Stop once a focused check supports the cause; do not repeat equivalent probes.\n"
...
def build_debug_user_message(block_name: str, phase: str = "sim",
    ...
    return (
        ...
        f"## Working Files\n"
        f"Available evidence (read selectively as directed below):\n"
        f"{files}"
        f"{instructions}"
